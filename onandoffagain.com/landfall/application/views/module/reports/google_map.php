...
<script type="text/javascript">
	...
	google.maps.event.addListener(map, 'click', function(event) {
	    ...
	    $(".st_light_lat_loc").attr('value', mouseLocation.k);
	    $(".st_light_long_loc").attr('value', mouseLocation.A);
...
//		    icon: 'http://onandoffagain.com/landfall/public/img/map-pin-green-md.png',
	...
	google.maps.event.addListener(element, 'click', function() {
	    $(".st_light_defect_id option").each(function() {
	    ...
	    $(".st_light_id").attr('value', element.id);
	    $(".st_light_active option").each(function() {
	    ...
	    bootbox.dialog({
		message: $('#st_light_update_form_div_holder').html(),
		title: 'What to do?',
		buttons: {
		    cancel: {
			label: 'Cancel',
			...
			callback: function() {
				...
				$.each($objs, function(obj_index, obj_element) {
				    if (obj_element.name == "st_light_defect_id") {
					element.defect_id = obj_element.value;
				    } else if (obj_element.name == "st_light_active") {
