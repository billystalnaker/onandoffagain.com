...
// Select user data to be displayed.
    ...
    function get_defect_types($defect_type_id = 0){
        ...
        if($defect_type_id <= 0){
            $this->data['defect_types'] = $this->db->select($sql_select)->get('defect_type')->result_array();
        }else{
            $filters                   = array('id' => $defect_type_id);
            $this->data['defect_type'] = $this->db->where($filters)->select($sql_select)->get('defect_type')->result_array();
        }
    }
...
// Set any returned status/error messages.
    ...
    function update_st_light_defect($st_light_id, $ajax = false){
        ...
        if($this->input->post('update_st_light_defect') || $this->input->is_ajax_request()){
            ...
            foreach($this->input->post('update') as $row){
                $deleting    = false;
                $has_comment = (trim($row['comment']) != '');
                if($row['current_status'] != $row['new_status'] || $has_comment){
                    // Insert new st_light_defect.
                    if($row['new_status'] == 1 || $has_comment){
                        $tmp = $this->insert_st_light_defect($st_light_id, $row['id'], ['comment' => trim($row['comment'])]);
                        $ret = (!$ret)?false:$tmp;
                    }
                ...
                if($has_comment && !$deleting){
                    $this->db->where(['st_light_id' => $st_light_id, 'defect_id' => $row['id']])->update('st_light_defect', ['comment' => trim($row['comment'])]);
                }
            }
...
// Redirect user.
    ...
    public function get_st_light_report(){
        ...
        $pdf->Cell(0, 12, 'Street Light Report', 0, 0, 'C');
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(0, 12, date('Y-m-d', time()), 0, 1, 'R');
...
//        var_dump($st_lights);
        ...
        foreach($header as $key => $head){
            $pdf->Cell($key, 7, $head, 1, 0, 'C', true);
        }
        $pdf->Ln();
        ...
        foreach($st_lights as $st_light_id => $defects){
            ...
            foreach($defects as $defect_id => $comment){
                ...
                foreach([60 => $st_light_name, 65 => $defect_name, 70 => $comment] as $width => $text){
                    $nb = max($nb, $pdf->NbLines($width, $text));
                }
                $h = 5 * $nb;
                ...
                //Draw the border
                $pdf->Rect($x, $y, 60, $h);
                $pdf->MultiCell(60, 5, $st_light_name, 'LR', 'L', $fill);
                $pdf->SetXY($x + 60, $y);
                //Save the current position
                $x    = $pdf->GetX();
                $y    = $pdf->GetY();
                //Draw the border
                $pdf->Rect($x, $y, 65, $h);
                $pdf->MultiCell(65, 5, $defect_name, 'LR', 'L', $fill);
                $pdf->SetXY($x + 65, $y);
                //Save the current position
                $x    = $pdf->GetX();
                $y    = $pdf->GetY();
                //Draw the border
                $pdf->Rect($x, $y, 70, $h);
                $pdf->MultiCell(70, 5, $comment, 'LR', 'L', $fill);
                $pdf->SetXY($x + 70, $y);
                $pdf->Ln($h);
                $fill = !$fill;
            }
